...
import { CATEGORY_NAME_VALIDATOR } from "@/lib/validators/catregory-validator"
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const cat = user.eventCategories.find(
    ...
    const eventData = {
      title: `${cat.emoji ?? "🔔"} ${cat.name.charAt(0).toUpperCase() + cat.name.slice(1)}`,
      description:
        validationResult.deacription ?? `A new ${cat.name} event has occurred`,
      color: cat.color,
